package com.imkrisna.samsung.nservice;
...
/**
 * <p>
 * This class is the main process for {@link com.imkrisna.samsung.nservice} library for android. NSConnection maintain the
...
public class NSConnection {
	...
	public void searchDevices(){
		if (!this.isWifiConnected()){
			Log.w(NSConnection.DEBUG_TAG + ".searchDevices", "Aborting Search: Wireless Network Not Connected");
			return;
		}
		...
		Thread explorer = new Thread(new NSRunnable(this) {
			...
			public void run() {
				...
				try{				
					Log.d(NSConnection.DEBUG_TAG + ".searchDevices", "Explorer #" + String.valueOf(randomId) + " Started!");
					
					this.executor.clearDevices();
					this.executor.discovery.search();
				}
				catch (Exception e){
					Log.e(NSConnection.DEBUG_TAG + ".searchDevices", "Explorer #" + String.valueOf(randomId) + " Error: " + e.getMessage());
				}
			}
		});
			...
			public void run() {
				...
				while (this.executor.isConnected()){
					try{
						...
						Log.d(NSConnection.DEBUG_TAG + ".connect", "Poller From: " + this.executor.getTvAddress());
						Log.d(NSConnection.DEBUG_TAG + ".connect", "-- Poller URL: " + request.getURI().toString());
						Log.d(NSConnection.DEBUG_TAG + ".connect", "-- Response: " + responsePhrase + " (" + String.valueOf(responseCode) + ")");
						
						switch (responseCode){
							case 200:
								this.executor.setLostCount(0);
								this.executor.notifyMessageReceived(EntityUtils.toString(response.getEntity()));
								break;
							case 408:
								this.executor.setLostCount(0);
								break;
						...
						Log.e(NSConnection.DEBUG_TAG + ".poller", "ClientProtocolException: " + e.getMessage());
					}
					catch (IOException e){
						Log.e(NSConnection.DEBUG_TAG + ".poller", "IOException: " + e.getMessage());
					}
				}
			}
		});
	...
	public void connect(String tvAddress){
		if (!this.isWifiConnected()) {
			Log.w(NSConnection.DEBUG_TAG + ".connect", "Aborting Connect: Wireless Network Not Connected");
			return;
		}
		...
		Thread connector = new Thread(new NSRunnable(this) {
			...
			public void run() {
				...
				try{
					...
					Log.d(NSConnection.DEBUG_TAG + ".connect", "Connecting to: " + this.executor.getTvAddress());
					Log.d(NSConnection.DEBUG_TAG + ".connect", "-- Connect URL: " + request.getURI().toString());
					Log.d(NSConnection.DEBUG_TAG + ".connect", "-- Response: " + responsePhrase + " (" + String.valueOf(responseCode) + ")");
					...
					Log.e(NSConnection.DEBUG_TAG + ".connect", "ClientProtocolException: " + e.getMessage());
					this.executor.notifyConnectionFailed(NSConnection.CONN_EXCEPTION);
				}
				catch (IOException e){
					Log.e(NSConnection.DEBUG_TAG + ".connect", "IOException: " + e.getMessage());
					this.executor.notifyConnectionFailed(NSConnection.CONN_EXCEPTION);
				}
			}			
		});
	...
	public void disconnect(){
		if (!this.isConnected()) {
			Log.w(NSConnection.DEBUG_TAG + ".disconnect", "Aborting Disconnect: Wireless Network Not Connected");
			return;
		}
		...
		Thread disconnector = new Thread(new NSRunnable(this) {
			...
			public void run() {
				...
				try{
					...
					Log.d(NSConnection.DEBUG_TAG + ".disconnect", "Disconnecting from: " + this.executor.getTvAddress());
					Log.d(NSConnection.DEBUG_TAG + ".disconnect", "-- Disconnect URL: " + request.getURI().toString());
					Log.d(NSConnection.DEBUG_TAG + ".disconnect", "-- Response: " + responsePhrase + " (" + String.valueOf(responseCode) + ")");
					...
					Log.e(NSConnection.DEBUG_TAG + ".disconnect", "ClientProtocolException: " + e.getMessage());
				}
				catch (IOException e){
					Log.e(NSConnection.DEBUG_TAG + ".disconnect", "IOException: " + e.getMessage());
				}
			}
		});
	...
	public void sendMessage(final String message){
		if (!this.isConnected()) {
			Log.w(NSConnection.DEBUG_TAG + ".sendMessage", "Aborting Send Message: Wireless Network Not Connected");
			return;
		}
		...
		Thread messager = new Thread(new NSRunnable(this) {
			...
			public void run() {
				...
				try{
					...
					Log.d(NSConnection.DEBUG_TAG + ".sendMessage", "Sending Message To: " + this.executor.getTvAddress());
					Log.d(NSConnection.DEBUG_TAG + ".sendMessage", "-- Sending Message URL: " + request.getURI().toString());
					Log.d(NSConnection.DEBUG_TAG + ".sendMessage", "-- Response: " + responsePhrase + " (" + String.valueOf(responseCode) + ")");
					...
					Log.e(NSConnection.DEBUG_TAG + ".sendMessage", "UnsupportedEncodingException: " + e.getMessage());
					this.executor.notifyMessageSendFailed(NSConnection.CONN_EXCEPTION);
				}
				catch (ClientProtocolException e){
					Log.e(NSConnection.DEBUG_TAG + ".sendMessage", "ClientProtocolException: " + e.getMessage());
					this.executor.notifyMessageSendFailed(NSConnection.CONN_EXCEPTION);
				}
				catch (IOException e){					
					Log.e(NSConnection.DEBUG_TAG + ".sendMessage", "IOException: " + e.getMessage());
					this.executor.notifyMessageSendFailed(NSConnection.CONN_EXCEPTION);
				}
			}
		});
	...
	public boolean isWifiConnected(){				 
		...
		if (ssid == null || ssid.contains("unknown") || ssid.isEmpty() || ssid.equals("0x")) return false;
		else return true;		 
	}
